...
const char * const BITCOIN_CONF_FILENAME = "poix.conf";
const char * const BITCOIN_PID_FILENAME = "poixd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "poix" is a composite category enabling all Poix-related debug output
            if(ptrCategory->count(string("poix"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PoixCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PoixCore
    // Mac: ~/Library/Application Support/PoixCore
    // Unix: ~/.poixcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PoixCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PoixCore";
#else
    // Unix
    return pathRet / ".poixcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty poix.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override poix.conf
